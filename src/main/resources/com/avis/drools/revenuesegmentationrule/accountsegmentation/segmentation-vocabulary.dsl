...
[condition][]- AWD Type is {AWD} = buildupAWDtype=="{AWD}"
...
[condition][]- Divisions is in {categories} =checkoutDivisions in ({categories})
[condition][]- Divisions is not in {categories} =checkoutDivisions not in ({categories})
[condition][]- Check AWD Type is in {categories} =buildupAWDtype in ({categories})
...
[condition][]- STC code is beginning with {listOfString} =eval(checkSTCode(bookingData,new String[] \{{listOfString}\} ))
